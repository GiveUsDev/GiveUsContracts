...
describe("Crowdfunding Contract", function () {
  ...
  async function deployCrowdfundingFixture() {
    ...
    const crowdfunding = await ethers.getContractAt("Crowdfunding", await crowdfundingProxy.getAddress());

    // Fixtures can return anything you consider useful for your tests
    return { Crowdfunding, crowdfundingProxy, crowdfunding, owner, addr1, addr2 };
  }
  ...
  async function createProjectDataAndUSDCFixture() {
    const { Crowdfunding, crowdfunding, crowdfundingProxy, owner, addr1, addr2 } = await loadFixture(deployCrowdfundingFixture);
    ...
    return { Crowdfunding, crowdfunding, crowdfundingProxy, owner, addr1, addr2, projectData, thresholds, emptyThresholds, USDC, usdc, projectDataZero };
  }
  ...
  async function createProjectAndUSDCFixture() {
    const { Crowdfunding, crowdfunding, crowdfundingProxy, owner, addr1, addr2, projectData, thresholds, emptyThresholds, USDC, usdc } = await loadFixture(createProjectDataAndUSDCFixture);
    ...
    return { Crowdfunding, crowdfunding, crowdfundingProxy, owner, addr1, addr2, projectData, thresholds, emptyThresholds, USDC, usdc };
  }
  ...
  describe("Upgrade", function () {
    it('Should Upgrade to V2', async () => {
      const { crowdfundingProxy, usdc } = await loadFixture(createProjectDataAndUSDCFixture);
      const CrowdfundingV2 = await ethers.getContractFactory("CrowdfundingV2");

      const crowdfundingUpgraded = await upgrades.upgradeProxy(await crowdfundingProxy.getAddress(), CrowdfundingV2);
      expect(await crowdfundingUpgraded.getAddress()).to.be.equal(await crowdfundingProxy.getAddress());
  ...
  describe("Deployment", function () {
    ...
    describe("pause()", function () {
      it("should not pause, revert : Access Control", async () => {
        ...
        await expect(crowdfunding.connect(addr1).pause()).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });
      ...
      it("should not pause twice, revert : Pausable: paused", async () => {
        ...
        await expect(crowdfunding.pause()).to.be.revertedWithCustomError(Crowdfunding, "EnforcedPause");
      });
    ...
    describe("createProject(Project calldata _project)", function () {
      it("Should not create project, revert : Missing Role", async function () {
        const { Crowdfunding, crowdfunding, owner, addr1, addr2, projectData, thresholds } = await loadFixture(createProjectDataAndUSDCFixture);
        await expect(crowdfunding.connect(addr1).createProject(projectData, thresholds)).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });
      ...
      it("Should not create project, revert : Token not supported", async function () {
        const { Crowdfunding, crowdfunding, owner, addr1, addr2, projectData, thresholds } = await loadFixture(createProjectDataAndUSDCFixture);
        await expect(crowdfunding.createProject(projectData, thresholds)).to.be.revertedWithCustomError(Crowdfunding, 'TokenNotSupported');
      });
      ...
      it("Should not create project, revert : Need at least one Threshold", async function () {
        ...
        await crowdfunding.addNewSupportedToken(usdc.getAddress());
        await expect(crowdfunding.createProject(projectData, emptyThresholds)).to.be.revertedWithCustomError(Crowdfunding, 'ZeroThresholds');
      });
      ...
      it("Should not create project, revert : ZeroRequiredAmount", async function () {
        ...
        await crowdfunding.addNewSupportedToken(usdc.getAddress());
        await expect(crowdfunding.createProject(projectDataZero, thresholds)).to.be.revertedWithCustomError(Crowdfunding, 'ZeroRequiredAmount');
      });
    ...
    describe("addNewSupportedToken(address tokenAddress)", function () {
      it("Should not add token, revert : AccessControl is missing role", async function () {
        ...
        await expect(crowdfunding.connect(addr1).addNewSupportedToken(tokenAddress)).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });
      ...
      it("Should not add token, revert : Zero address ", async function () {
        ...
        await expect(crowdfunding.addNewSupportedToken(ZERO_ADDRESS)).to.be.revertedWithCustomError(Crowdfunding, 'ZeroAddress');
      });
    ...
    describe("withdrawFees(address tokenAddress)", function () {
      it("Should not withdrawFees, revert : AccessControl is missing role", async function () {
        const {Crowdfunding, crowdfunding, owner, addr1, addr2 } = await loadFixture(createProjectAndFundFixture);
        const tokenAddress = owner.address;
        await expect(crowdfunding.connect(addr1).withdrawFees(tokenAddress)).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });
      ...
      it("Should withdrawFees and emit event withdrawFees ", async function () {
        ...
        await expect(crowdfunding.withdrawFees(await usdc.getAddress(), { from: owner.address }))
          ...
          .withArgs(owner.address, await usdc.getAddress(), availableFees);
      });
    });
    describe("donateToProject(uint id, uint amount)", function () {
      ...
      it("Should donate even if goal is reached and make is available to withdraw", async function () {
        ...
        const donationAmount = 100000;
        let donatedAmountMinusFee = BigInt(donationAmount - (donationAmount * projectData.donationFee / 10000));
        ...
        const expectedAmount = currentAmount + donatedAmountMinusFee;

        const expectedAvailableToWithdraw = availableToWithdraw + donatedAmountMinusFee;
    ...
    describe("voteForThreshold(uint256 id, bool vote)", function () {
      ...
      it("Shouldnt vote, revert : Can Only Vote Once", async function () {
        const { Crowdfunding, crowdfunding, owner, projectID } = await loadFixture(createProjectAndStartedVotingFixture);
        await crowdfunding.voteForThreshold(projectID, true);
        await expect(crowdfunding.voteForThreshold(projectID, true)).to.be.revertedWithCustomError(Crowdfunding, 'CanOnlyVoteOnce');
      });
    ...
    describe("endThresholdVoting(uint256 id)", function () {
      ...
      it("Shouldnt end voting, revert : You are not allowed", async function () {
        const {Crowdfunding, crowdfunding, addr1, projectID } = await loadFixture(createProjectAndStartedVotingFixture);
        await expect(crowdfunding.connect(addr1).endThresholdVoting(projectID)).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });
    ...
    describe("withdrawFunds(address exchangeTokenAddress)", function () {
      ...
      it("Should withdrawFunds", async function () {
        ...
        const shouldNewBalance = balance + availableToWithdraw;
        ...
        expect(newAvailableToWithdraw).to.be.equal(0);
      });
    ...
    describe("setDonationFee(uint projectId, uint16 newFee)", function () {
      ...
      it("Shouldnt change DonationFee, revert : Access Control", async function () {
        const {Crowdfunding, crowdfunding, addr1, projectID } = await loadFixture(createProjectAndDonateFixture);
        const newFee = 100;
        await expect(crowdfunding.connect(addr1).setDonationFee(projectID, newFee)).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });
    ...
    describe("UpdateProjectStatus(uint256 projectId, bool newStatus)", function () {
      it("Shouldnt UpdateProjectStatus, revert : Access Control", async function () {
        const {Crowdfunding, crowdfunding, addr1, projectID } = await loadFixture(createProjectAndDonateFixture);
        await expect(crowdfunding.connect(addr1).updateProjectStatus(projectID, false)).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });

      it("Shouldnt UpdateProjectStatus, revert : Invalid Project Id", async function () {
        const { Crowdfunding, crowdfunding, owner, projectID } = await loadFixture(createProjectAndDonateFixture);
        await expect(crowdfunding.updateProjectStatus(100, false)).to.be.revertedWithCustomError(Crowdfunding, 'InvalidProjectId');
      });
      ...
      it("Should UpdateProjectStatus, emit ProjectStatusUpdated Event", async function () {
        ...
        expect(await crowdfunding.updateProjectStatus(projectID, false))
    ...
    describe("UpdateProjectVoteCooldown(uint256 projectId, uint256 newCooldown)", function () {
      it("Shouldnt UpdateProjectVoteCooldown, revert : Access Control", async function () {
        const {Crowdfunding, crowdfunding, addr1, projectID } = await loadFixture(createProjectAndDonateFixture);
        await expect(crowdfunding.connect(addr1).updateProjectVoteCooldown(projectID, 0)).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });
    ...
    describe("getThresholdVoteFromAddress(address voterAddress, uint256 projectId, uint256 thresholdId)", function () {
      ...
      it("Should getThresholdVoteFromAddress", async function () {
        ...
        const thresholdVote = await crowdfunding.getThresholdVoteFromAddress(owner.address, projectID, 0);
    ...
    describe("withdrawFundsToOtherProject(uint256 fromProjectID, uint256 toProjectID)", function () {
      ...
      it("Shouldnt withdrawFundsToOtherProject, revert : Access Control", async function () {
        const { Crowdfunding, crowdfunding, addr1, projectID } = await loadFixture(createProjectAndDonateFixture);
        await expect(crowdfunding.connect(addr1).withdrawFundsToOtherProject(projectID, projectID)).to.be.revertedWithCustomError(Crowdfunding, "AccessControlUnauthorizedAccount");
      });
      ...
      it("Shouldnt withdrawFundsToOtherProject, revert : ProjectNotActive", async function () {
        ...
        const firstProjectId = 0;
        const seconsProjectId = 1;
        await crowdfunding.updateProjectStatus(seconsProjectId, false);
      ...
      it("Should withdrawFundsToOtherProject", async function () {
        ...
        expect(secondProjectAfter.currentAmount).to.be.equal(currentAmountFirstProject + currentAmountSecondProject);
